...
## 로그에 보이는 스레드 이름: `nio-*` vs `pool-*`는 무엇인가

동시 write 문제를 디버깅할 때, 로그의 스레드 이름이 힌트가 되는 경우가 많다.  
(스레드 이름 자체가 "원인"은 아니지만, **서로 다른 실행 컨텍스트가 같은 세션에 겹쳐서 write하고 있는지**를 빠르게 보여주는 강한 단서다.)

대표적으로 아래 두 종류를 자주 본다.

| 스레드 이름 예시 | 보통 의미 | 누가 관리하나 | 언제 등장하나 |
|---|---|---|---|
| `nio-8080-exec-2` (또는 `http-nio-8080-exec-2`) | Tomcat NIO 커넥터의 요청/이벤트 처리 워커 스레드 | **컨테이너(Tomcat)** | WebSocket 핸드셰이크, 메시지 콜백 처리 등 |
| `pool-2-thread-1` | `ExecutorService`/스케줄러 등에서 만든 스레드 풀 스레드 | **애플리케이션(내 코드)** | `ScheduledExecutorService`, `@Async`, 커스텀 스레드 풀 작업 등 |

즉, 흔한 패턴은 이렇다.

- `nio-...` 같은 컨테이너 스레드에서 **"즉시 응답(ACK)"**을 보내고
- `pool-...` 같은 백그라운드 스레드 풀에서 **추가 메시지**를 보내는 순간
- 두 전송이 타이밍상 겹치면 `TEXT_PARTIAL_WRITING` 같은 예외가 날 수 있다.

> 중요한 점: 스레드 이름 규칙은 서버/환경에 따라 달라질 수 있다.  
> 이름이 `nio-*`/`pool-*`이 아니어도 본질은 같다.  
> "컨테이너가 관리하는 스레드" vs "애플리케이션이 만든 스레드 풀"처럼 **송신 경로가 둘 이상**이면 언제든 경합이 발생할 수 있다.
...
- 로그에 `nio-*`/`pool-*`처럼 서로 다른 스레드가 섞여 보이면, **동일 세션에 대한 동시 write 경로**를 먼저 의심하는 게 빠르다.
